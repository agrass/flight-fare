...
class SearchQuery
  ...
  def open_file
    ...
    File.open("dest.txt") do |file|
      file.each do |line|
        dest,price = line.split(",")
        @@dest << dest
        @@alert_price[desr] = price
      end
    end
  end
  ...
  def send_email(msg)
    Mail.deliver do
      to 'antoniograssb@gmail.com'
      from 'ag4ss@gmail.com '
      subject 'price alert'
      body msg
    end
  end
...
class LanParser < PriceParser
  ...
  def get_values(orig, dest, date1, date2)
    ...
    json['data']['recomendations']['data'].each do |data|
